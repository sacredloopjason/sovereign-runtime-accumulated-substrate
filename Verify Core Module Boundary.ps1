...
$parentCommit = 'e304bb0ca4a29c76e76233d2e466d99bc1fc338c'
$python = Join-Path $project 'python311\python.exe'
$verifier = Join-Path $project 'verify_core_module_boundary.py'
$evidenceDirectory = Join-Path $project 'evidence\core_module_boundary'
...
& $python $verifier --model-dir $ModelDirectory --evidence-dir $evidenceDirectory
if ($LASTEXITCODE -ne 0) {
    throw "CORE module boundary verifier exited with code $LASTEXITCODE."
}
...
$integrity = [ordered]@{
    schema = 'CORE_MODULE_BOUNDARY_POST_EXECUTION_INTEGRITY_V1'
...
if ($runtimeEvidence.outcome -ne 'CORE_MODULE_BOUNDARY_CLOSED') {
